...
},
{
  ...
  "number": "4.1",
  "title": "線形空間",
  "body": " 線形空間   線形空間と部分空間   が以下の条件を満すとき、 を 線形空間 であるという。  .  .   線形空間 が を満すとき は の 部分空間 であるという。    は線形空間であり、 の部分空間である。 また、任意の線形空間 について、 と は の部分空間である。 これらは の 自明な部分空間 という。   線形空間と部分空間の例   任意の について は線形空間であり、 の部分空間である。  任意の について は線形空間であり、 の部分空間である。  は線形空間であり、 の部分空間である。     スパン   任意の について、 と定義する。 これを のスパン もしくは が貼る線形空間(部分空間) という。 また、 と定義する。    "
},
{
  "id": "sec-subspace-2",
  "level": "2",
...
},
{
  ...
  "number": "4.3",
  "title": "リストとアルゴリズム",
  "body": " リストとアルゴリズム  ベクトルの集合の要素に順序がついていると便利なので、順序付きの集合としてリストを導入する。   リスト   について、ベクトルの列 をベクトルの リスト という。 括弧を省略して と表すこともある。      リスト が線形従属 ある が存在して、 .    は より明らか。 を示す。  が線形従属であることから、「すべて0」ではない が存在して  を である最大のものとすると、 よって、       任意の と と について、 を と定義する。このとき、任意の について、  .  が線形独立  が線形独立。     任意の を固定する。 のとき、 がすべての について成り立つので、補題の結論は自明に成り立つ。以下では を仮定する。  まず は自明である。また、 とすることで、元の が復元できるので、同様に である。 よって は示された。  次に と仮定する。このとき、 である。よって、 とおくと、 である。  ここで また、 なので   よって が示された。     ガウスの消去法    入力:  出力: で任意の について以下の三条件を満たすもの。  .  が線形独立  が線形独立。   .     アルゴリズムの疑似コード    for :  If : continue (2行目に戻る)  の非零成分のインデックス(どれでもよい)   for :   return     このアルゴリズムの正当性を証明する。  まず、スパンと線形独立性が保存されていることを証明する。 このアルゴリズムの中で を更新しているが、それは5行目と7行目である。 5行目の更新はベクトルに非零のスカラー倍をする操作であるが、明らかにスパンや線形独立性は変化しない。 7行目の更新は よりスパンや線形独立性を変えない。  次に を仮定する。スパンの保存条件より が成り立つ。 したがって である。 よって、ある が存在し、 が成り立つ。 ここで のとき、 の中で 成分が非零なのは だけである。よって、 である。 したがって、 である( のときも明らかに成り立つ)。 同じ議論を繰り返すことで、 であることが分かる。       次元数ベクトル空間の線形独立なベクトルの個数は 以下。    が線形独立であるとすると、 はすべての について、ピボット を与えるが、これは互いに異なるため、 である。    "
},
{
  "id": "sec-list-alg-3",
  "level": "2",
...
},
{
  "id": "sec-list-alg-4",
  "level": "2",
  "url": "sec-list-alg.html#sec-list-alg-4",
  "type": "補題",
  "number": "4.3.2",
  "title": "",
  "body": "  リスト が線形従属 ある が存在して、 .    は より明らか。 を示す。  が線形従属であることから、「すべて0」ではない が存在して  を である最大のものとすると、 よって、   "
},
{
  "id": "lem-elem-vec",
  "level": "2",
  ...
  "number": "4.3.3",
  "title": "",
  "body": "   任意の と と について、 を と定義する。このとき、任意の について、  .  が線形独立  が線形独立。     任意の を固定する。 のとき、 がすべての について成り立つので、補題の結論は自明に成り立つ。以下では を仮定する。  まず は自明である。また、 とすることで、元の が復元できるので、同様に である。 よって は示された。  次に と仮定する。このとき、 である。よって、 とおくと、 である。  ここで また、 なので   よって が示された。   "
},
{
  "id": "alg-lin-gauss",
  "level": "2",
  ...
  "number": "4.3.4",
  "title": "ガウスの消去法.",
  "body": " ガウスの消去法    入力:  出力: で任意の について以下の三条件を満たすもの。  .  が線形独立  が線形独立。   .     アルゴリズムの疑似コード    for :  If : continue (2行目に戻る)  の非零成分のインデックス(どれでもよい)   for :   return     このアルゴリズムの正当性を証明する。  まず、スパンと線形独立性が保存されていることを証明する。 このアルゴリズムの中で を更新しているが、それは5行目と7行目である。 5行目の更新はベクトルに非零のスカラー倍をする操作であるが、明らかにスパンや線形独立性は変化しない。 7行目の更新は よりスパンや線形独立性を変えない。  次に を仮定する。スパンの保存条件より が成り立つ。 したがって である。 よって、ある が存在し、 が成り立つ。 ここで のとき、 の中で 成分が非零なのは だけである。よって、 である。 したがって、 である( のときも明らかに成り立つ)。 同じ議論を繰り返すことで、 であることが分かる。   "
},
{
  "id": "thm-vec-ind",
  "level": "2",
...
},
{
  ...
  "number": "4.4",
  "title": "基底と次元",
  "body": " 基底と次元   基底   線形空間 について が の基底     が線形独立        任意の線形独立な と について は線形独立。    ある が を満たすとする。 と仮定すると、 より、 となり矛盾。 したがって であるが、このとき の線形独立性より、 である。 よって、 は線形独立である。     基底の延長定理   任意の線形空間 と線形独立なベクトル について、 を含む の基底が存在する。    の基底を以下のアルゴリズムで構成できる。   while :  の任意に選んだ要素   上記のアルゴリズムの中で に要素を追加していくが、 より常に線形独立である。 より、 の要素数は高々 なので、このアルゴリズムは停止する。  このアルゴリズムは抽象的なものである。2,3行目の具体的な計算方法は線形空間 が具体的にどのように表されているかに依存する。    基底の延長定理を空集合( )の場合に適用することで以下の系を得る。   任意の線形空間 は基底を持つ。        について、 に含まれる線形独立集合のサイズは高々 である。    とおく。 が線形独立であると仮定する。 このとき、 を仮定して矛盾を導く。  リスト は線形従属であり を貼るので、 を適用することで、少なくとも一つのベクトルが削除された の基底が得られる。 これを繰替えすことで を順番に追加していく。 このとき、 は線形独立なので削除されることはない。 より、 を追加する前に はすべて削除されている。 よって、 の部分集合が の基底になる。 このとき、 となるが、これは の線形独立性に矛盾。 よって、 である。       任意の線形空間 について、 の基底のサイズは一定。    と を の基底とする。 は線形独立であり、 は を貼るので より、 である。同様に なので、 である。     次元   線形空間 について、その基底のサイズを の次元 という。      数ベクトル と がそれぞれ線形独立であり とする。 このとき、ある が存在し、 は線形独立である。    ある が存在し、 のとき、 より、 は線形独立である。 すべての について と仮定する。このとき より、 であるので矛盾。      任意の線形空間 とその二つの異なる基底 について、ある と が存在して、      "
},
{
  "id": "sec-basis-2",
  "level": "2",
...
},
{
  "id": "lem-ind-add",
  "level": "2",
  "url": "sec-basis.html#lem-ind-add",
  "type": "補題",
  "number": "4.4.2",
  "title": "",
...
},
{
  ...
  "number": "4.4.7",
  "title": "次元.",
  "body": " 次元   線形空間 について、その基底のサイズを の次元 という。   "
},
{
  "id": "sec-basis-10",
  "level": "2",
  ...
  "number": "4.4.8",
  "title": "",
  "body": "  数ベクトル と がそれぞれ線形独立であり とする。 このとき、ある が存在し、 は線形独立である。    ある が存在し、 のとき、 より、 は線形独立である。 すべての について と仮定する。このとき より、 であるので矛盾。   "
},
{
  "id": "sec-basis-11",
  "level": "2",
  ...
  "number": "4.4.9",
  "title": "",
  "body": "  任意の線形空間 とその二つの異なる基底 について、ある と が存在して、     "
}
]
